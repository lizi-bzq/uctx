...
{
    fc1.stack.ss_sp = sf1;
    fc1.stack.ss_size = sizeof(sf1);
    fc1.uc_link = NULL;
    fc2.stack.ss_sp = sf2;
    fc2.stack.ss_size = sizeof(sf2);
    fc2.uc_link = NULL;
    if (makeuctx(&fc1, float_coro, &fc1))
        return 1;
    if (makeuctx(&fc2, float_coro, &fc2))
        return 1;
    swapuctx(&fm, &fc1);
    swapuctx(&fm, &fc2);
    return fc_r1 != fc_r2;
...
{
    ic1.stack.ss_sp = si1;
    ic1.stack.ss_size = sizeof(si1);
    ic1.uc_link = NULL;
    ic2.stack.ss_sp = si2;
    ic2.stack.ss_size = sizeof(si2);
    ic2.uc_link = NULL;
    if (makeuctx(&ic1, int_coro, &ic1))
        return 1;
    if (makeuctx(&ic2, int_coro, &ic2))
        return 1;
    swapuctx(&im, &ic1);
    swapuctx(&im, &ic2);
    return ic_r1 != ic_r2;
...
static void c_b(void *arg)
{
    link_log = 2;
}
static void c_a(void *arg)
{
    link_log = 1;
}
...
{
    ca.stack.ss_sp = sa;
    ca.stack.ss_size = sizeof(sa);
    ca.uc_link = &cb;
    cb.stack.ss_sp = sb;
    cb.stack.ss_size = sizeof(sb);
    cb.uc_link = &cm;
    if (makeuctx(&ca, c_a, NULL))
        return 1;
    if (makeuctx(&cb, c_b, NULL))
        return 1;
    link_log = 0;
    swapuctx(&cm, &ca);
    return link_log != 2;
...
{
    struct uctx bad;
    if (makeuctx(NULL, float_coro, NULL) != -1)
        return 1;
    if (makeuctx(&bad, NULL, NULL) != -1)
        return 1;
    return 0;
}
...
{
    pc1.stack.ss_sp = sp1;
    pc1.stack.ss_size = sizeof(sp1);
    pc1.uc_link = NULL;
    pc2.stack.ss_sp = sp2;
    pc2.stack.ss_size = sizeof(sp2);
    pc2.uc_link = NULL;
